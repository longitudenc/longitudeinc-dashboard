// lib/staffing-trend.ts
//
// STAFFING-TREND-v1, the computation. Moved out of app/api/gs/staffing-trend so
// Staffing needs (lib/staffing-needs.ts) reads short and idle hours from the
// SAME code the Staffing trends screen shows -- two definitions of "short"
// that disagreed by a rounding rule would be worse than one.
//
// Inputs are already scoped to the caller and, if wanted, to one salon. See the
// route for the reasoning behind each measure; the comments travel with it.
...
export function computeStaffingTrend(demand: any[], punches: any[], shifts: any[], salon = '') {
